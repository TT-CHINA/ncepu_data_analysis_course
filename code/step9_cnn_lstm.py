...
step9_cnn_lstm.py
CNN-LSTM 混合模型 - 差分预测
CNN 提取局部时序特征，LSTM 学习长程依赖（公共逻辑来自 utils.py）
"""
import os
import sys
import time
...
model = utils.CNNLSTMModel(input_size=X_train_seq.shape[2]).to(device)
...
model.load_state_dict(torch.load('models/cnn_lstm_best.pth'))
model.eval()
test_preds_diff = []
with torch.no_grad():
...
axes[0].set_title('CNN-LSTM 训练损失曲线', fontsize=13, fontweight='bold')
axes[0].legend(); axes[0].grid(alpha=0.3)
axes[1].plot(test_dates_seq, truths, label='真实值', linewidth=2, color='steelblue')
axes[1].plot(test_dates_seq, preds_absolute, label='CNN-LSTM预测', linewidth=2, linestyle='--', color='purple')
axes[1].set_title(f"CNN-LSTM 测试集预测 (RMSE={m['RMSE']:.0f}, MAPE={m['MAPE']:.2f}%, R²={m['R2']:.3f})",
                  fontsize=13, fontweight='bold')
axes[1].set_xlabel('日期'); axes[1].set_ylabel('日总用电量 (MWh)')
axes[1].legend(); axes[1].grid(alpha=0.3)
plt.tight_layout()
plt.savefig('figures/14_cnn_lstm_prediction.png', dpi=150, bbox_inches='tight')
plt.close()

pd.DataFrame({'date': test_dates_seq, 'truth': truths, 'cnn_lstm_pred': preds_absolute}).to_csv(
    'data/cnn_lstm_results.csv', index=False)
pd.DataFrame([{'model': 'CNN-LSTM (Differencing)', **m,
               'training_time': training_time,
               'params': sum(p.numel() for p in model.parameters())}]).to_csv(
    'data/cnn_lstm_metrics.csv', index=False)
